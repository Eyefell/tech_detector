...
#### 暗号化 / TLS
- HTTPS 使用状況・CryptCheck グレード
- TLS バージョン（1.0〜1.3）・非推奨プロトコル警告
- 前方秘匿性（Forward Secrecy）判定
- 暗号スイート名・鍵長
- HTTP バージョン（HTTP/1.1, HTTP/2, HTTP/3）
- HSTS 設定状況・HSTS Preload リスト登録状況
- OCSP ステープリング

#### 証明書情報（CryptCheck + certspotter）
- 発行元 CA・発行元 DN
- Subject DN（コモンネーム / 組織名 / 部署名 / 国 等）
- 鍵アルゴリズム・鍵長・ECC 曲線名
- 署名アルゴリズム
- 有効期間・残り日数・失効警告
...
#### TLS 脆弱性スキャン（CryptCheck）
- POODLE (SSL 3.0) / SSLv2 / FREAK / Sweet32 / RC4 / DES
- NULL 暗号 / Anonymous Cipher / SHA-1 署名

#### PQC（耐量子暗号）対応状況
- 鍵交換アルゴリズムの PQC 対応チェック
- 証明書署名アルゴリズムの PQC 対応チェック
...
```
background.js (Service Worker / Firefox: background script)
  ├── アイコンクリック → ポップアップウィンドウを中央に生成
  ├── HTTP レスポンスヘッダーからの技術検出
  ├── content script のプログラム的インジェクション
  ├── CryptCheck API プロキシ（リダイレクト対応）
...
| `FETCH_CRYPTCHECK` | popup.js → background.js | CryptCheck API プロキシ（リダイレクト対応） |
...
| API | 用途 | タイムアウト |
|-----|------|------------|
| [CryptCheck API](https://cryptcheck.fr/) | TLS グレード・暗号スイート・脆弱性・証明書情報 | 10秒 |
| [certspotter API](https://sslmate.com/certspotter/) | 証明書透明性ログ（SANs・有効期間・発行元） | 6秒 |
| [HSTS Preload API](https://hstspreload.org/) | HSTS プリロードリスト登録状況 | 4秒 |
...
| ブラウザ | サポート状況 |
|---------|------------|
| Chrome / Chromium | Manifest V3 完全対応（`background.service_worker`） |
| Firefox | 109.0 以上、Manifest V3（`background.scripts`）、`browser-polyfill.js` による互換レイヤー |
...
- **タイムアウト制御** — 外部 API には個別のタイムアウトを設定（4〜10秒）
